...
def plot_all(orig_path: str, enc_path: str, save_path: str = "visualisation.png"):
    ...
    for bit in range(8):
        plane = (channel >> bit) & 1
        planes.append(plane.mean())

    bars = ax6.bar(range(8), planes,
                   color=['#e74c3c' if b == 0 else '#3498db' for b in range(8)],
...
if __name__ == "__main__":
    if len(sys.argv) == 3:
        ...
        if not os.path.exists(orig_path):
            print("  Files not found. Run 'python steganography.py' first.")
            print("  Or: python steg_visualise.py <original.png> <encoded.png>")
            sys.exit(1)
